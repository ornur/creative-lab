In the project use next.js, tailwindcss v4, shadcn ui, pnpm, motion(framer-motion)

# Create a website for the creative lab

    Website should be modern and creative with dark mode and smooth animations and responsive for all devices language kazakh
    - Landing page:
        - Hero section
        - About section
        - Speakers section
        - Schedule section
        - Pricing section
        - Contact section
    -
    - Images:
        - Hero section use hero.webp
        - Header component use logo.webp and add navigation links for the sections
        - Speakers section use images start  with person... and end with .webp
    -
    - Section Pricing and Contact:
        - Hero section hero image should above Prism component and below the header component and others
        - Pricing section tickets are default with 19990 tenge strikethrough discount until 15 April ticket price is 9990 tenge with cool animation
        - Contanct section has form with fields: name, email, phone, message and submit button and handle to the Whatsapp number +7 700 728 0546
    -

# Website text for the users to read and understand about the creative lab

```txt
Creative Lab деген не?

Creative Lab - бұл классикалық “мастер-класс” емес.
Бұл - креатив индустриясындағы адамдар жиналатын тірі орта (хаб)

Мұнда:
	•	құрғақ теория жоқ
	•	“вода” ақпарат жоқ
	•	бірнәрсе зорлап сату жоқ

Тек тәжірибе, нақты кейстер және ашық әңгіме.

Creative Lab - идея алмасатын, ой кеңейтетін және өз ортаңды табатын орын.


Онда қандай пайда аласыз?
Creative Lab-қа келген адам:
Пайдалы инсайттар алады
	•	Нарықта не жұмыс істеп жатыр, не істемей жатыр
	•	Қазіргі трендтер емес, шын нәтиже беретін тәжірибе

Нетворкинг
	•	Сіз сияқты дамып жатқан адамдар
	•	Болашақ серіктестер, клиенттер, команда
Жаңа көзқарас қалыптастырады
	•	Өз салаңызға басқа ракурстан қарауды үйренесіз
	•	Креатив ойлау деңгейіңіз өседі

Тікелей сұрақ қою мүмкіндігі
	•	Спикерлермен еркін формат
	•	“Сахна - сұрақ - жауап” емес, диалог

Кімдерге арналған?
Creative Lab:
	•Контент жасаушыларға (видео, фото, SMM)
	•Маркетологтарға
	•Дизайнерлерге
	•	Продюсерлер мен режиссерларға
	•	Креатив индустрияға кіргісі келетін жастарға
	•	Жалпы “өсу керек”, “жаңа орта керек” деп жүрген кез келген адамға


Өз орныңды алып үлгер:
Ватсап
```
