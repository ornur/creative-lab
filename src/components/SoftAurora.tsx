...
float perlin3D(float amplitude, float frequency, float px, float py, float pz) {
  ...
  vec3 g001 = gradientHash(vec3(fx, fy, fz + 1.0)); // Fixed hash index for z
...
export default function SoftAurora({
  ...
  useEffect(() => {
    ...
    // 1. Initialize Program as CONST immediately
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        uTime: { value: 0 },
        uResolution: { value: [gl.canvas.width, gl.canvas.height, 1.0] },
        uSpeed: { value: speed },
        uScale: { value: scale },
    ...
    // 2. Define resize logic (Now it can safely reference 'program')
    function resize() {
